...
class CancelOrStopIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        if user_selected_obj == 0 or alexa_selected_obj == 0:
            self.data_apl["statData"]["sources"][0]["AvTimeAlexa"] = f"0:00 min"
            self.data_apl["statData"]["sources"][0]["AvTimeUser"] = f"0:00 min"
        else:
        ...
        if user_selected_obj == 0 or alexa_selected_obj == 0:
            speak_output = choose_utterance(mood, "bye") + f"""Du hast leider keine Objekte erraten. Trotzdem vielen Dank fürs Spielen!"""
        else:
